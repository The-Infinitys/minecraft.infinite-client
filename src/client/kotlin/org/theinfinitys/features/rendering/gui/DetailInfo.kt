...
class DetailInfo : ConfigurableFeature(initialEnabled = false) {
        ...
        if (scanTargetBlockEntity != null) {
            ...
            val inventoryType =
                when (entity) {
                    ...
                    is BrewingStandBlockEntity -> InventoryType.BREWING // 追加
            ...
            if (entity is ChestBlockEntity) {
                ...
                if (chestType != ChestType.SINGLE) {
                    ...
                    if (otherState.block == Blocks.CHEST && otherState.get(ChestBlock.CHEST_TYPE) != ChestType.SINGLE) {
                        ...
                        scannedInventoryData[leftPos] = InventoryData(inventoryType, firstHalf)
                        scannedInventoryData[rightPos] = InventoryData(inventoryType, secondHalf)
                    } else {
    ...
    fun saveData() {
        val dataDir = getDataDirectory().toFile()
        if (!dataDir.exists()) {
            dataDir.mkdirs()
        }
        ...
        for ((pos, invData) in scannedInventoryData) {
    ...
    fun loadData() {
        ...
        dataDir.listFiles()?.filter { it.name.startsWith("chunk_") && it.name.endsWith(".json") }?.forEach { file ->
            ...
            for ((posKey, serialData) in chunkData.inventories) {
                ...
                scannedInventoryData[pos] = InventoryData(invType, items)
            }
        }
    }
