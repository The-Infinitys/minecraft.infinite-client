...
class RelativeMoveToAction(
    x: Int,
    z: Int,
    val y: Int? = null,
    val tolerance: Double = 0.5,
) : AiAction() {
    private val startPos: Vec3d = playerPos ?: Vec3d.ZERO
    private val targetX: Int = (startPos.x + x).toInt()
    private val targetZ: Int = (startPos.z + z).toInt()
    private val targetPos: Vec3d = Vec3d(targetX + 0.5, y?.toDouble() ?: player?.y ?: 0.0, targetZ + 0.5) // Center of the block
    ...
    override fun tick() {
        ...
        if (currentPos.distanceTo(targetPos) > tolerance) {
            ControllerInterface.press(client.options.forwardKey) {
                (playerPos?.distanceTo(targetPos) ?: Double.MAX_VALUE) > tolerance
            }
        } else {
    ...
    override fun onSuccess() {
        println("Successfully moved relatively to $targetX, $targetZ")
        ControllerInterface.release(client.options.forwardKey)
    ...
    override fun onFailure() {
        println("Failed to move relatively to $targetX, $targetZ")
        ControllerInterface.release(client.options.forwardKey)
        ControllerInterface.release(client.options.backKey)
        ControllerInterface.release(client.options.leftKey)
        ControllerInterface.release(client.options.rightKey)
