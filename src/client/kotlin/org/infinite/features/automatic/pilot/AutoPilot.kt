...
class AutoPilot : ConfigurableFeature(initialEnabled = false) {
    ...
    // 現在の飛行速度 (m/s)
    val flySpeed: Double
        get() = InfiniteClient.playerInterface.movement.speed() * 20 // 実際の実装に合わせる

    // Y方向の速度
    val riseSpeed: Double
        get() = (player?.velocity?.y ?: 0.0) * 20
    val height: Double
        get() = player?.y ?: 0.0
    val pitch: Double
...
class AutoPilotCondition(
    ...
    private fun handleRiseFlying(): AimTaskConditionReturn {
        val isRising = autoPilot.riseSpeed > 0 || autoPilot.flySpeed > 10
        autoPilot.aimTaskCallBack =
            if (autoPilot.isDisabled()) {
                AimTaskConditionReturn.Failure
            } else if (isRising) { // **修正: MinSpeed より早い場合**
                AimTaskConditionReturn.Exec // まだ目標速度に達していない -> 実行を継続 (上昇を継続)
            } else {
                InfiniteClient.log("${autoPilot.height}")
                AimTaskConditionReturn.Success // 目標速度 MinSpeed に達した -> 成功として AimTask を終了
            }
    ...
    private fun handleFallFlying(): AimTaskConditionReturn {
        ...
        autoPilot.aimTaskCallBack =
            if (autoPilot.isDisabled()) {
                AimTaskConditionReturn.Failure
            } else if (autoPilot.flySpeed < targetSpeed) {
                AimTaskConditionReturn.Exec // まだ目標速度に達していない -> 実行を継続 (下降を継続)
            } else {
                AimTaskConditionReturn.Success // 目標速度 MaxSpeed 以上になった -> 成功として AimTask を終了
            }
        return autoPilot.aimTaskCallBack!!
    }
}
    ...
    override val roll: CameraRoll
        get() {
            return CameraRoll(
                calculateTargetYaw(),
                when (state) {
                    PilotState.Landing -> 10.0 // 着陸
                    PilotState.FallFlying -> 16.0 // 下降
                    PilotState.RiseFlying -> -40.0 // 上昇
                    else -> 0.0 // その他の状態
                },
            )
        }
